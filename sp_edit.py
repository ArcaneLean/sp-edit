...
All dump-* commands accept --json for machine-readable output.
    ...
    python3 sp_edit.py move-task "Title or ID" "AfterTaskTitle or ID"
    ...
    python3 sp_edit.py add-note "Content" [project]
...
def _find_note(state, title_or_id):
    """Match note by ID or content (first line)."""
    notes = state.get("note", {}).get("entities", {})
    if title_or_id in notes:
        return title_or_id, notes[title_or_id]
    needle = title_or_id.lower()
    def first_line(n):
        return n.get("content", "").split("\n")[0].lower()
    matches = [(nid, n) for nid, n in notes.items() if first_line(n) == needle]
    if len(matches) == 1:
        return matches[0]
    if len(matches) > 1:
        sys.exit(f"Error: multiple notes match '{title_or_id}'")
    partial = [(nid, n) for nid, n in notes.items() if needle in first_line(n)]
    if len(partial) == 1:
        return partial[0]
    if len(partial) > 1:
        sys.exit(f"Error: multiple notes partially match '{title_or_id}'")
    sys.exit(f"Error: no note found matching '{title_or_id}'")


def add_note(content, project_name=None):
    parsed = pull()
    state = parsed["state"]
    now_ms = int(time.time() * 1000)
    ...
    print(f"Created note{proj_str} (id={note_id})")
...
def update_note(title_or_id, changes):
    ...
    _save(parsed)
    push()
    print(f"Updated note '{note_id}': {changes}")
...
def delete_note(title_or_id):
    ...
    _make_op(parsed, "DEL", "NOTE", "ND", {
        "id": note_id,
        "projectId": note.get("projectId"),
        "isPinnedToToday": note.get("isPinnedToToday", False),
    }, note_id)
...
def pin_note(title_or_id):
    ...
    _save(parsed)
    push()
    print(f"Pinned note '{note_id}' to today")
...
def add_subtask(title, parent_title_or_id):
    ...
    _make_op(parsed, "CRT", "TASK", "TA", {
        "task": {k: v for k, v in task.items() if k != "parentId"},
        "parentId": parent_id,
    }, task_id)
...
def complete_subtask(title_or_id):
    ...
    if not task.get("parentId"):
        sys.exit(f"Error: '{task['title']}' is not a subtask")
    ...
    _save(parsed)
    push()
    print(f"Completed subtask '{task['title']}'")
...
def move_to_backlog(title_or_id):
    ...
    _make_op(parsed, "MOV", "TASK", "PRB", {
        "taskId": task_id,
        "afterTaskId": None,
        "workContextId": proj_id,
    }, task_id)
...
def move_from_backlog(title_or_id):
    ...
    _make_op(parsed, "UPD", "TASK", "HU", {
        "task": {"id": task_id, "changes": {"isDone": False}},
    }, task_id)
...
def unschedule_task(title_or_id):
    ...
    _make_op(parsed, "UPD", "TASK", "HSX", {"id": task_id}, task_id)
...
def reschedule_task(title_or_id, new_date):
    ...
    subtasks = [state["task"]["entities"][s] for s in task.get("subTaskIds", [])
                if s in state["task"]["entities"]]
    _make_op(parsed, "UPD", "PLANNER", "LP", {
        "task": {**task, "subTasks": subtasks},
        "day": new_date,
        "isShowSnack": False,
    }, task_id)
...
def _wm_op(parsed, state, task_id, proj_id, after_task_id):
    _make_op(parsed, "MOV", "PROJECT", "WM", {
        "taskId": task_id,
...
def move_task_to_top(title_or_id):
    ...
    proj_id = task.get("projectId")
    if not proj_id or proj_id not in state["project"]["entities"]:
        sys.exit(f"Error: task '{task['title']}' has no project.")
    task_ids = state["project"]["entities"][proj_id]["taskIds"]
    if task_id not in task_ids:
        sys.exit(f"Error: task '{task['title']}' is not in the active task list.")
    task_ids.remove(task_id)
    task_ids.insert(0, task_id)
    _wm_op(parsed, state, task_id, proj_id, None)
    _save(parsed)
    push()
    print(f"Moved '{task['title']}' to top")


def move_task(title_or_id, after_title_or_id):
    parsed = pull()
    state = parsed["state"]
    task_id, task = _find_task(state, title_or_id)
    after_id, after_task = _find_task(state, after_title_or_id)
    proj_id = task.get("projectId")
    if not proj_id or proj_id not in state["project"]["entities"]:
        sys.exit(f"Error: task '{task['title']}' has no project.")
    task_ids = state["project"]["entities"][proj_id]["taskIds"]
    if task_id not in task_ids:
        sys.exit(f"Error: task '{task['title']}' is not in the active task list.")
    task_ids.remove(task_id)
    after_idx = task_ids.index(after_id)
    task_ids.insert(after_idx + 1, task_id)
    _wm_op(parsed, state, task_id, proj_id, after_id)
    _save(parsed)
    push()
    print(f"Moved '{task['title']}' after '{after_task['title']}'")
...
if __name__ == "__main__":
    ...
    if cmd == "pull":
    ...
    elif cmd == "move-task" and len(args) > 2:
        move_task(args[1], args[2])
        ...
        add_note(args[1], args[2] if len(args) > 2 else None)
